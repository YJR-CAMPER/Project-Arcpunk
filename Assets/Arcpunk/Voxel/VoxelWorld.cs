...
namespace Arcpunk.Voxel
{
    public class VoxelWorld : MonoBehaviour
    {
        ...
        [Header("Optimization")]
        [SerializeField] private int _maxRebuildPerFrame = 2;      // 프레임당 최대 리빌드 수
        ...
        /// <summary>프레임당 제한된 수의 더티 청크만 리메싱 (콜라이더 포함).</summary>
        private void ProcessDirtyChunks()
        {
            int rebuilt = 0;
            foreach (var kvp in _chunks)
            {
                if (rebuilt >= _maxRebuildPerFrame) break;
                ...
                {
                    renderer.UpdateMesh(mesh); // 메시 + 콜라이더 즉시
                    EnsureDualMaterial(renderer);
                }
        ...
        /// <summary>메시의 서브메시 수에 맞게 머티리얼 할당.</summary>
        private void EnsureDualMaterial(ChunkRenderer renderer)
        {
            ...
            if (subMeshCount >= 2 && _crossMaterial != null)
            {
                // 크로스 빌보드 있음 → 머티리얼 2개
                mr.sharedMaterials = new Material[] { _chunkMaterial, _crossMaterial };
            }
            else
            {
                // 크로스 없음 → 머티리얼 1개만
                mr.sharedMaterials = new Material[] { _chunkMaterial };
            }
        }
